...
def train(agent, out_model_file, int_output_dir):
    agent.train_controller(out_model_file=out_model_file,int_output_dir=int_output_dir)
    return
...
def test_panda3d(agent):
    agent.test_controller_panda3d()
    return
...
def test_no_agent(env):
    ...
    viewer = MocapPandaViewer(env)

    runner = EpisodeRunner(env)
    runner.__enter__()  # with 문을 쓰지 않고 직접 enter

    def step_env(task):
        # done이면 마무리
        if runner.done:
            runner.__exit__(None, None, None)
            # 창을 닫고 싶으면 아래 중 하나 선택
            # viewer.userExit()
            return task.done
        ...
        # 2) frame_skip 만큼 env state 계산 (기존 while 내부 로직 그대로)
        ...
        # ✅ (다음 단계) 여기에서 Panda3D 렌더러 업데이트를 호출하면 됩니다.
        viewer.update_from_env()

        return task.cont

    # ✅ 여기만 “taskMgr.add() <-- 이부분 수정” 입니다.
    viewer.taskMgr.add(step_env, "step_env")

    viewer.run()  # blocking (이제 env 진행은 step_env가 담당)
...
def run(rank, num_procs, args):
    ...
    print("MODEL_CONFIG:", model_config_file)
