...
class AMDM(model_base.BaseModel):
    ...
    def _build_model(self, config):
        self.diffusion = GaussianDiffusion(config, self.dataset)
...
class GaussianDiffusion(nn.Module):
    ...
    def __init__(
        ...
        self.dataset = dataset
        self.cfg_scale = config["diffusion"].get("cfg_scale", 3.0)
    ...
    def build_uncond_input(self, x):
        """
        x : [B, D] 또는 [B, T, D]
        root + footstep(cartesian + polar) 부분만 0으로 지운
        'unconditional' 입력을 만든다.
        """
        x_uncond = x.clone()

        ds = self.dataset

        # 1) root (dx, dy, heading 등) 앞부분
        '''if getattr(ds, "data_root_dim", 0) > 0:
            root_dim = int(ds.data_root_dim)
            x_uncond[..., :root_dim] = 0.0'''

        # 2) footstep cartesian (left_dx, left_dz, right_dx, right_dz)
        if hasattr(ds, "foot_cartesian") and ds.foot_cartesian is not None:
            s_c, e_c = ds.foot_cartesian
            s_c, e_c = int(s_c), int(e_c)
            x_uncond[..., s_c:e_c] = 0.0

        return x_uncond

    def predict_eps_cfg(self, last_x, x, te):
        B = x.shape[0]
        ...
        # 2) uncond input (root+foot 제거)
        x_uncond = self.build_uncond_input(x)
        last_uncond = last_x  # last_x는 그대로 컨텍스트
        te_uncond = te

        # 3) 배치로 합치기
        x_cat = torch.cat([x_cond, x_uncond], dim=0)  # [2B, D]
        last_cat = torch.cat([last_cond, last_uncond], dim=0)  # [2B, D]
        te_cat = torch.cat([te_cond, te_uncond], dim=0)  # [2B, Tdim]

        # 4) 모델 1번만 호출
        eps_cat = self.model(last_cat, x_cat, te_cat)  # [2B, D]
    ...
    def sample_ddpm(self, last_x, extra_info, record_process=False):
        ...
        for t in range(self.T - 1, -1, -1):
            ...
            #pred = self.predict_eps_cfg(last_x, x, te)
