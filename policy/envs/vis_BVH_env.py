...
from render.realtime.my_second_mocap_renderer import PBLMocapViewer
import gymnasium as gym
import numpy as np

class vis_BVH_env(gym.Env):
    NAME = "vis_BVH_env"
    def __init__(self, config, model, dataset, device):  
        ...
        '''self.root_facing = torch.full(
            (self.num_parallel, 1),  # shape
            math.pi,
            device=self.device
        )'''
        self.root_xz = torch.zeros((self.num_parallel, 2)).to(dtype = torch.float32, device = self.device)
        '''self.root_xz = torch.tensor([[0.71494926, 0.17642220]] * self.num_parallel, dtype=torch.float32, device=self.device)'''
