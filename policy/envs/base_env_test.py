...
class EnvBase(gym.Env):
    def __init__(self, config, model, dataset, device):
        ...
        self.max_timestep = config.get('max_timestep_test', 2000) if self.is_rendered else config.get('max_timestep',
                                                                                                      1000 // self.frame_skip)
        ...
        self.timestep = torch.zeros((self.num_parallel, 1), dtype=torch.long, device=self.device)
        self.substep = torch.zeros((self.num_parallel, 1), dtype=torch.long, device=self.device)
