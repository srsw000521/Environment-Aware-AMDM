...
class PBLMocapViewer:
    ...
    #def render(self, xyzs, facings, root_xzs, lfoot, rfoot, lfoot_polar, rfoot_polar, time_remain, action):
    def render(self, xyzs, facings, root_xzs, lfoot, rfoot, time_remain, action):
        ...
        lfoot = (lfoot * FOOT2METER).squeeze(0).detach().cpu().numpy()  # (H,3)
        rfoot = (rfoot * FOOT2METER).squeeze(0).detach().cpu().numpy()  # (H,3)
        '''lfoot_polar = (lfoot_polar * FOOT2METER).squeeze(0).detach().cpu().numpy()
        rfoot_polar = (rfoot_polar * FOOT2METER).squeeze(0).detach().cpu().numpy()  # (H,3)'''

        for index in range(self.num_characters):

            #self.characters.set_joint_positions_polar(joint_xyzs[index], index, lfoot, rfoot, lfoot_polar, rfoot_polar, self.env.clip_timestep)
            self.characters.set_joint_positions(joint_xyzs[index], index, lfoot, rfoot,self.env.clip_timestep)
        ...
        if keys.get(ord("d")) == RELEASED:
            ...
            while True:
                ...
                if keys.get(ord(" ")) == RELEASED:
                    ...
                    if self.env.clip_timestep >= self.env.timestep_end - 1:
                        print("[INFO] End of motion.")
                    else:
                        frame = self.env.get_next_frame()
                        _, reward, done, info = self.env.calc_env_state(frame)
                elif keys.get(65295) == RELEASED:  # left key
                    if self.env.clip_timestep <= 0:
                        print("[INFO] Start of motion.")
                    else:
                        frame = self.env.get_previous_frame()
                        _, reward, done, info = self.env.calc_env_state(frame)

        elif keys.get(ord("1")) == RELEASED:
            if self.env.file_idx <= 0:
                print("[INFO] Start of BVH.")
            else:
                self.env.clip_timestep = 0
                self.env.file_idx -= 1
                self.env.reset_index()
            print(f'BVH_IDX: {self.env.file_idx}')
        elif keys.get(ord("3")) == RELEASED:
            if self.env.file_idx >= 59:
...
class MultiMocapCharacters:
    ...
    def set_joint_positions_polar(self, xyzs, index, lfoot, rfoot, lfoot_polar, rfoot_polar, cur_frame):
        ...
        if not hasattr(self, "lfoot_marker"):
            sphere_shape = self._p.createVisualShape(pb.GEOM_SPHERE, radius=0.05, rgbaColor=[0, 0, 1, 1])  # 파랑
        ...
        if not hasattr(self, "rfoot_marker"):
            sphere_shape = self._p.createVisualShape(pb.GEOM_SPHERE, radius=0.05, rgbaColor=[1, 0, 0, 1])  # 빨강
            self.rfoot_marker = self._p.createMultiBody(baseVisualShapeIndex=sphere_shape, basePosition=[0, 0, 0])

        if not hasattr(self, "lfoot_marker_polar"):
            sphere_shape = self._p.createVisualShape(pb.GEOM_SPHERE, radius=0.05, rgbaColor=[0, 0, 0.5, 1])
            self.lfoot_marker_polar = self._p.createMultiBody(baseVisualShapeIndex=sphere_shape, basePosition=[0, 0, 0])

        if not hasattr(self, "rfoot_marker_polar"):
            sphere_shape = self._p.createVisualShape(pb.GEOM_SPHERE, radius=0.05, rgbaColor=[0.5, 0, 0, 1])
            self.rfoot_marker_polar = self._p.createMultiBody(baseVisualShapeIndex=sphere_shape, basePosition=[0, 0, 0])

        lfoot_pos3d = [float(lfoot[0]), float(lfoot[1]), 0.0]
        rfoot_pos3d = [float(rfoot[0]), float(rfoot[1]), 0.0]

        lfoot_pos3d_polar = [float(lfoot_polar[0]), float(lfoot_polar[1]), 0.0]
        rfoot_pos3d_polar = [float(rfoot_polar[0]), float(rfoot_polar[1]), 0.0]
        ...
        self._p.resetBasePositionAndOrientation(self.lfoot_marker_polar, posObj=lfoot_pos3d_polar, ornObj=(0, 0, 0, 1))
        self._p.resetBasePositionAndOrientation(self.rfoot_marker_polar, posObj=rfoot_pos3d_polar, ornObj=(0, 0, 0, 1))

        self._p.configureDebugVisualizer(pb.COV_ENABLE_RENDERING, 1)

    def set_joint_positions(self, xyzs, index, lfoot, rfoot, cur_frame):
